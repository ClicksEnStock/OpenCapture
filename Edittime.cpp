...
enum {
	...
	PROPID_FLIP_HORIZONTAL_ENABLED,
	PROPID_BACKGROUNDCOLOR,
	PROPID_DEBUGPARAMETER_TITLE,
	PROPID_DEBUG_SHOW_TRACKING,
// Example
// -------
//	PROPID_TEXTTITLE,	
//	PROPID_TEXT,	
...
//	PropData_Color		(PROPID_COLOR,		IDS_PROP_COLOR,			IDS_PROP_COLOR_INFO),
	//PropData_
	PropData_Group			(PROPID_DEFAULTPARAMETER_TITLE,	IDS_MAINPARAMETERS, IDS_MAINPARAMETERS),
	PropData_CheckBox		(PROPID_FLIP_HORIZONTAL_ENABLED, IDS_PROP_FLIP_HORIZONTAL_ENABLED,	IDS_PROP_FLIP_HORIZONTAL_ENABLED_INFO),
	PropData_Color			(PROPID_BACKGROUNDCOLOR,IDS_PROP_BACKGROUNDCOLOR,IDS_PROP_BACKGROUNDCOLOR_INFO),
	PropData_Group			(PROPID_DEBUGPARAMETER_TITLE,	IDS_DEBUGPARAMETERS, IDS_DEBUGPARAMETERS),
	PropData_CheckBox		(PROPID_DEBUG_SHOW_TRACKING,IDS_PROPID_DEBUG_SHOW_TRACKING,IDS_PROPID_DEBUG_SHOW_TRACKING_INFO),
...
int WINAPI DLLExport CreateObject(mv _far *mV, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
	{
		...
		edPtr->swidth = 100;
		edPtr->sheight = 100;
		edPtr->isFlipped = false;
		edPtr->backgroundColor= RGB(200,200,200);
		edPtr->isDebugShowTracking = false;
...
// Returns background color.
	case PROPID_BACKGROUNDCOLOR:
		return new CPropDWordValue(edPtr->backgroundColor);
//
//	// Returns a string
//	case PROPID_TEXT:
...
//	case PROPID_CHECK:
//		return edPtr->nCheck;
	case PROPID_FLIP_HORIZONTAL_ENABLED:
		return (bool)edPtr->isFlipped;
		break;
	case PROPID_DEBUG_SHOW_TRACKING:
		return (bool)edPtr->isDebugShowTracking;
		break;
	}
...
void WINAPI DLLExport SetPropValue(LPMV mV, LPEDATA edPtr, UINT nPropID, LPVOID lParam)
{
	...
	switch (nPropID) {
		...
		edPtr->backgroundColor = ((CPropDWordValue*)pValue)->m_dwValue;
		break;
//
//	case PROPID_COMBO:
...
//		break;
	case PROPID_FLIP_HORIZONTAL_ENABLED:
		edPtr->isFlipped = (nCheck==0) ? false : true;
		mvInvalidateObject(mV, edPtr);
		mvRefreshProp(mV, edPtr, PROPID_FLIP_HORIZONTAL_ENABLED, TRUE);
		break;
	case PROPID_DEBUG_SHOW_TRACKING:
		edPtr->isDebugShowTracking = (nCheck==0) ? false : true;
		mvInvalidateObject(mV, edPtr);
		mvRefreshProp(mV, edPtr, PROPID_DEBUG_SHOW_TRACKING, TRUE);
		break;
	}
#endif // !defined(RUN_ONLY)
}
